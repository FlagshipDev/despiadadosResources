	...
	for i=1, #Status, 1 do
		if minimal then
			...
			if Status[i].name == "thirst" then
				sed = Status[i].val
			elseif Status[i].val == "hunger" then
				hambre = Status[i].val
			end
		else
			table.insert(status, {
...
function GetStatusData2(minimal)
	local status = {}
	local hs = {}
	local sed = 0
	local hambre = 0
	...
	for i=1, #Status, 1 do
		if minimal then
			...
			if Status[i].name == "thirst" then
				sed = Status[i].val
			elseif Status[i].name == "hunger" then
				hambre = Status[i].val
...
AddEventHandler('esx_status:load', function(status)
  ...
  Citizen.CreateThread(function()
  	...
  	--	})
		...
		TriggerEvent('hud:onTick', GetStatusData2(true))
