...
AddEventHandler('esx_eden_clotheshop:pay', function()
	...
	xPlayer.removeMoney(Config.Clothes.Price)

	TriggerClientEvent('esx:showNotification', source, _U('you_paid') .. Config.Clothes.Price)
end)
...
ESX.RegisterServerCallback('esx_eden_clotheshop:checkMoney', function(source, cb)
	...
	if xPlayer.get('money') >= Config.Clothes.Price then
		cb(true)
	else
		cb(false)
	end
