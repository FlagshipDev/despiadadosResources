        ...
        if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
          ...
          if locked == 1 or class == 15 or class == 16 or class == 14 then
            ...
            if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
              ...
              OpenMaletero(GetVehicleNumberPlateText(vehFront), vehCapacity)
            end
...
function OpenMaletero(plate,max)
  ESX.TriggerServerCallback('esx_trunk:getInventoryV', function(inventory)
    local elements = {}
    table.insert(elements, {label = 'Coger objetos', value = 'take_objects'})
    table.insert(elements, {label = 'Depositar objetos', value = 'put_objects'})
    table.insert(elements, {label = 'Coger armas', value = 'take_armas'})
    table.insert(elements, {label = 'Depositar armas', value = 'put_armas'})
    table.insert(elements, {label = 'Coger dinero negro', value = 'take_bm'})
    table.insert(elements, {label = 'Depositar dinero negro', value = 'put_bm'})
    
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'car_inventory1',
      {
      ...
      function(data, menu)
        ...
        if data.current.value == 'take_objects' then
          OpenCogerObjetos(plate, max)
        elseif data.current.value == 'put_objects' then
          OpenDejarItems(owner,plate,max,inventory.weight)
        elseif data.current.value == 'take_armas' then
          OpenCogerArmas(plate, max)
        elseif data.current.value == 'put_armas' then
          OpenDejarArmas(owner,plate,max,inventory.weight)
        elseif data.current.value == 'take_bm' then
          ESX.UI.Menu.Open(
            'dialog', GetCurrentResourceName(), 'get_item_count',
            {
              title = '¿Cantidad? - '..inventory.blackMoney..'$',
            },
            function(data2, menu)
              ...
              if quantity == nil or quantity < 1 then
                ...
                TriggerServerEvent('esx_trunk:getItem', plate, 'item_account', 'black_money', quantity)

                ESX.SetTimeout(500, function()
                  ESX.UI.Menu.CloseAll()
                  OpenMaletero(plate,max)
                end)
              end
            end, function(data2,menu)
              menu.close()
            end)
        elseif data.current.value == 'put_bm' then
          OpenDejarDineroNegro(owner,plate,max,inventory.weight)
        end
...
function OpenCogerObjetos(plate,max)
  ESX.TriggerServerCallback('esx_trunk:getInventoryV', function(inventory)
    local plate = plate
    ...
    for i=1, #inventory.items, 1 do
      ...
      if item.count > 0 then
        table.insert(elements, {label = item.label .. ' - ' .. item.count, type = 'item_standard', value = item.name, haveImage = true})
      end
...
function OpenCogerArmas(plate,max)
  ESX.TriggerServerCallback('esx_trunk:getInventoryV', function(inventory)
    local plate = plate
    local owner= PlayerPedId()
    local elements = {}
    ...
    for i=1, #inventory.weapons, 1 do
      ...
      table.insert(elements, {label = ESX.GetWeaponLabel(weapon.name) .. ' con ' .. weapon.ammo .. ' balas', type = 'item_weapon', value = weapon.name, ammo = weapon.ammo, haveImage = true})
    end
...
function OpenDejarItems(owner,plate,max,weight)
  ...
  ESX.TriggerServerCallback('esx_trunk:getPlayerInventory', function(inventory)
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'player_inventory3',
      {
            ...
            function(data2, menu)
              ...
              if quantity == nil or quantity < 1 then
                ...
                ESX.SetTimeout(300, function()
                  ...
                  OpenDejarItems(playerPed,plate,max,weight)
                end)
              end
            end,
            function(data2,menu)
...
function OpenDejarArmas(owner,plate,max,weight)
  ...
  ESX.TriggerServerCallback('esx_trunk:getPlayerInventory', function(inventory)
    ...
    for i=1, #weaponList, 1 do
      ...
      if HasPedGotWeapon(playerPed,  weaponHash,  false) and weaponList[i].name ~= 'WEAPON_UNARMED' then
        ...
        table.insert(elements, {label = weaponList[i].label .. ' con ' .. ammo .. ' balas', type = 'item_weapon', value = weaponList[i].name, ammo = ammo, haveImage = true})
      end
      ...
      function(data, menu)
        ...
        if data.current.type == 'item_weapon' then
          ...
          ESX.SetTimeout(300, function()
            ...
            ESX.UI.Menu.CloseAll()
            OpenDejarArmas(playerPed,plate,max,weight)
          end)
        elseif data.current.type == 'retour' then
          ESX.UI.Menu.CloseAll()
          OpenDejarArmas(plate,max)
        else
...
function OpenDejarDineroNegro(owner,plate,max,weight)
  ...
  ESX.TriggerServerCallback('esx_trunk:getPlayerInventory', function(inventory)
    ...
    ESX.UI.Menu.Open(
      'dialog', GetCurrentResourceName(), 'put_item_count',
      {
        title = '¿Cuánto quieres depositar? - '..inventory.blackMoney..'$',
      },
      function(data2, menu)
        local quantity = tonumber(data2.value)
        ...
        if quantity == nil or quantity < 1 then
          ...
          menu.close()
          TriggerServerEvent('esx_trunk:putItem', plate, 'item_account', 'black_money', tonumber(data2.value),max, myVeh)
          ESX.SetTimeout(500, function()
            ESX.UI.Menu.CloseAll()
            OpenMaletero(plate,max)
          end)
