...
this.updateVehicle = function(data) {
	...
	if (data.status == true) {	
		if (vehicleInfo.classList.contains('inactive')) {
			...
			$("#speed3").fadeIn('slow',function(){
				$(this).animate({'bottom': '0.0vh'},'slow');
			});
			unfade(document.getElementById("speed3"));
		}
		...
		if (vehicleInfo.classList.contains('active')) {
			...
			vehicleInfo.classList.add('active');
			$("#speed3").fadeIn('slow',function(){
				$(this).animate({'bottom': '-20.0vh'},'slow');
			});
			fade(document.getElementById("speed3"));
