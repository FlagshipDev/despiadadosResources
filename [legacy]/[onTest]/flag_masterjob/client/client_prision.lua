...
function JailLogin()
	...
	ESX.ShowNotification("La última vez que te desconectaste estabas cumpliendo condena, por lo cual sigues en la cárcel!")
...
function UnJail()
	...
	ESX.Game.Teleport(PlayerPedId(), Config.Prision.Teleports["Boiling Broke"])

	ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
		TriggerEvent('skinchanger:loadSkin', skin)
	end)

	ESX.ShowNotification("Has salido de la cárcel, buena suerte !")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Tienes una condena de " .. jailTime .. " minutos!")
	...
	Citizen.CreateThread(function()
		while jailTime > 0 do
			...
			local Packages = Config.Prision.PrisonWork["Packages"]
...
function LoadTeleporters()
	Citizen.CreateThread(function()
		while true do
			...
			for p, v in pairs(Config.Prision.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Abrir la puerta", 0.4)
...
function PackPackage(packageId)
	local Package = Config.Prision.PrisonWork["Packages"][packageId]
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Embalando... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			local DeliverPosition = Config.Prision.PrisonWork["DeliverPackage"]
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Entregar el paquete", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Enviar a la cárcel", value = "jail_closest_player" },
				{ label = "Sacar a un jugador de la cárcel", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("El tiempo ha de ser en minutos!")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Tienes que poner algo!")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("La cárcel está vacía!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Condenado: " .. playerArray[i].name .. " | Condena: " .. playerArray[i].jailTime .. " minutos", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Soltar a alguien",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
