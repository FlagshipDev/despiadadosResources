    ...
    ESX.TriggerServerCallback('esx_ambulancejob:getDeathStatus', function(isDead)
		if isDead and bekleme then
            ESX.TriggerServerCallback('pazzodoktor:doktorsOnline', function(EMSOnline, hasEnoughMoney)
				if EMSOnline <= Config.AIDoctor.Doktor and hasEnoughMoney then
					TriggerEvent("pazzodoktor:canlan")
					--TriggerServerEvent('pazzodoktor:odeme')
					bekleme = false
				else
					if EMSOnline > Config.AIDoctor.Doktor then
...
function cagirma(DoktorP)
	if Config.AIDoctor.MythicProgbar then
...
function Tedavi(DoktorP)
	...
	notification(_U('treatment_done'), 'success')
	RemovePedElegantly(DoktorP)
	bekleme = true
end

function notification(text, type)
	if Config.AIDoctor.MythicNotify then
		if type == nil then
			type = 'inform'
		end
