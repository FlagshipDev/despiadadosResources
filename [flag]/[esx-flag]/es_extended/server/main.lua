Citizen.CreateThread(function()
	...
	while true do
		...
		SetConvarServerInfo('Creado por: ', 'FTP & Co.')
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		table.insert(tasks, function(cb)
			MySQL.Async.fetchAll('SELECT identifier, DATE_FORMAT(expire, "%d-%m-%Y") AS fecha FROM vip WHERE identifier = @identifier', {
				...
				local _fecha
				if result ~= nil and result[1] ~= nil then
					_fecha = result[1].fecha
				else
					_fecha = nil
				end
				userData.fechVip = _fecha
				cb(_fecha)
			end)
		end)
		...
		Async.parallel(tasks, function(results)
			local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job, userData.loadout, userData.playerName, userData.lastPosition, userData.isVip, userData.fechVip)
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	TriggerClientEvent('esx:giveAnimation', sourceXPlayer) 
