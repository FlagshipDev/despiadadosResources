...
TriggerEvent('chat:addSuggestion', Config.RecordCommandES, Config.RecordCommandSuggestES, {{ name="/guardar y /descartar para guardarlos/descartar clips", help="" }})
TriggerEvent('chat:addSuggestion', Config.OpenEditorCommandES, Config.OpenEditorCommandSuggestES, {{ name="/grabar y /descartar para grabar/descartar clips", help="" }})
TriggerEvent('chat:addSuggestion', Config.DiscardCommandES, Config.DiscardCommandSuggestES, {{ name="/grabar y /guardar para grabar/guardar clips", help="" }})
TriggerEvent('chat:addSuggestion', Config.SaveCommandES, Config.SaveCommandSuggestES, {{ name="/grabar, /guardar y /descartar para grabar/guardar y descartar clips", help="" }})


RegisterCommand(Config.RecordCommandES, function()
  StartRecording(1)
end)

RegisterCommand(Config.OpenEditorCommandES, function() 
  NetworkSessionLeaveSinglePlayer()
  ActivateRockstarEditor()
end)

RegisterCommand(Config.DiscardCommandES, function() 
  StopRecordingAndDiscardClip()
end)

RegisterCommand(Config.SaveCommandES, function() 
...
ESX.ShowFloatingHelpNotification = function(msg, coords)
    ...
    --SetFloatingHelpTextStyle(1, 2, 23, -1, 3, 0)
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	...
	elseif props.modSmokeEnabled == false then 
	...
	elseif props.modXenon == false then 
		ToggleVehicleMod(vehicle, 22, false)
	 end

	if props.modFrontWheels ~= nil then
		SetVehicleMod(vehicle, 23, props.modFrontWheels, props.modCustomWheels)
	end

	if props.modBackWheels ~= nil then
		SetVehicleMod(vehicle, 24, props.modBackWheels, props.modCustomWheels)
	end
	...
	if props.modDriftkit ~= nil and props.modDriftkit == 1 then
...
ESX.ShowInventory = function()
	...
	if ESX.PlayerData.money > 0 then
		...
		local cash = '<img class="ItemImage" onerror="this.style.display=\'none\'" src="https://leyendasrp.es/cash.png"/>'
		table.insert(elements, {
		--	label     = ('%s: <span style="color:green;">%s</span>'):format(_U('cash'), formattedMoney),
			label     = cash .. ('%s: %s'):format(_U('cash'), formattedMoney),
	...
	for i=1, #ESX.PlayerData.accounts, 1 do
		...
		--	local formattedMoney = _U('locale_currency', ESX.Math.GroupDigits(ESX.PlayerData.accounts[i].money))
			local formattedMoney = _U('locale_currency', ESX.Math.GroupDigits(ESX.Math.Round(ESX.PlayerData.accounts[i].money)))
			local canDrop = ESX.PlayerData.accounts[i].name ~= 'bank'
			local accountImg = ''
			if ESX.PlayerData.accounts[i].name == 'black_money' then
				accountImg = '<img class="ItemImage" onerror="this.style.display=\'none\'" src="https://leyendasrp.es/black_money.png"/>'
			...
			--if ESX.PlayerData.accounts[i].name ~= 'black_money' then
				table.insert(elements, {
					...
					label     = accountImg .. ('%s: %s'):format(ESX.PlayerData.accounts[i].label, formattedMoney),
	...
	for i=1, #ESX.PlayerData.inventory, 1 do
		if ESX.PlayerData.inventory[i] ~= nil and ESX.PlayerData.inventory[i].count ~= nil and ESX.PlayerData.inventory[i].count > 0 then
			table.insert(elements, {
		--		label     = ESX.PlayerData.inventory[i].label .. ' x' .. ESX.PlayerData.inventory[i].count,
				label 	  = ESX.PlayerData.inventory[i].label .. "  <span style='color:gray'>-</span> <span style='font-weight:500;color:rgb(255, 255, 255)'>" .. ESX.PlayerData.inventory[i].count .. '</span>',
				...
				isIllegal = ESX.PlayerData.inventory[i].isIllegal,
				haveImage = true
			})
	--		totalWeight = totalWeight + (ESX.PlayerData.inventory[i].count * ESX.PlayerData.inventory[i].weight)
		end
	end
	...
	{
	--	title    = _U('inventory'),
		title    = _U('inventory'), --.. ' ' .. (totalWeight/1000) .. '/' .. ESX.Math.Round(ESX.PlayerData.maxInventoryWeight/1000, 2) ..  ' Kg',
		align    = 'bottom-right',
		elements = elements,
		enableImages = true
	}, function(data, menu)
		...
		if data.current.canRemove then
			...
			if(data.current.type ~= 'item_weapon' and data.current.type ~= 'item_money'  )then
				...
				for i=1, #players, 1 do
					if players[i] ~= PlayerId() then
						...
						table.insert(elements, {
							...
							label = '[' .. GetPlayerServerId(players[i]) .. '] ' .. GetPlayerName(players[i]),
							player = players[i]
						})
					end
				end
