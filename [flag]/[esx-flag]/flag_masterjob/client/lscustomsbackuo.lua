...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if (PlayerData.job ~= nil and PlayerData.job.name == 'mechanic') or Config.LSCustoms.IsMechanicJobOnly == false then
				for k,v in pairs(Config.LSCustoms.Zones) do
					...
					if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < v.Size.x then
						...
						ESX.ShowFloatingHelpNotification('Presiona ~y~E~s~ para personalizar el vehículo', vector3(playerX, playerY, playerZ+0.9))
						break
					else
						isInLSMarker  = false
					end
